...
- [ ] A1. Initialise repository structure (`src/`, `calgo/`, `data/`, `exports/`)
- [ ] A2. Define all constants in `constants.py` with GitHub Actions secrets
- [ ] A3. Create `.env.example` with every required variable documented
- [ ] A4. Write `SECRETS.md` listing all GitHub Actions secrets with sources
- [ ] A5. Create `AGENTS.md` with delegation rules and behavioral constraints
- [ ] A6. Create `requirements.txt` (no TF/Torch/ONNX)
- [ ] A7. Add `.gitignore` covering models, CSVs, venvs, secrets
- [ ] A8. Set up GitHub Actions CI workflow (lint + test on push)
- [ ] A9. Write initial `README.md` with architecture overview
- [ ] A10. Configure `structlog` + `rich` logging pipeline
- [ ] A11. Add `pyproject.toml` for linting (ruff/flake8) and formatting
